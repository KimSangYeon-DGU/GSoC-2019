...
def factor_covariance(covariance):
  cov_lower = np.linalg.cholesky(covariance)
  inv_cov_lower = np.linalg.inv(cov_lower)

  inv_cov = np.dot(inv_cov_lower.T, inv_cov_lower)
  _, log_det_cov = np.linalg.slogdet(cov_lower)
  log_det_cov *= 2

  return inv_cov, log_det_cov

def log_probability(observations, mean, covariance, c):
  observations = np.asarray(observations.T)
  
  k = len(observations)
  inv_cov, log_det_cov = factor_covariance(covariance)

  diff = observations - mean
  v = np.dot(np.dot(diff, inv_cov), diff.T)
